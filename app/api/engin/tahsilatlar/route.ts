...
export async function GET(req: NextRequest) {
  ...
  const { data } = await supabase.from('mlk_engin_tahsilatlar').select('*').order('tarih', { ascending: false })
  return NextResponse.json(data || [])
}
export async function POST(req: NextRequest) {
  const user = getUser(req); if (!user || user.role === 'goruntule') return NextResponse.json({ error: 'Yetkisiz' }, { status: 403 })
  const body = await req.json()
  const { data, error } = await supabase.from('mlk_engin_tahsilatlar').insert(body).select().single()
